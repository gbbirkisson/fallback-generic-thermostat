...
from homeassistant.components.generic_thermostat.climate import (
    CONF_AC_MODE, CONF_COLD_TOLERANCE, CONF_HEATER, CONF_HOT_TOLERANCE,
    CONF_INITIAL_HVAC_MODE, CONF_KEEP_ALIVE, CONF_MAX_TEMP, CONF_MIN_DUR,
    CONF_MIN_TEMP, CONF_PRECISION, CONF_PRESETS, CONF_SENSOR, CONF_TARGET_TEMP,
    CONF_TEMP_STEP, DEFAULT_TOLERANCE, GenericThermostat)
from homeassistant.const import (CONF_NAME, CONF_UNIQUE_ID, PRECISION_HALVES,
...
async def async_setup_platform(
    ...
    if not keep_alive:
        ...
        _LOGGER.info(
            "Configuration variable 'keep_alive' not provided. Defaulting to %s",
            keep_alive,
        )
...
class FallbackGenericThermostat(GenericThermostat):
    def __init__(
        ...
        self.fallback_force_switch_entity_id = fallback_force_switch_entity_id
        ...
        if not keep_alive:
            _LOGGER.warning(
                "Value for keep_alive must be specified to use fallback_on_ratio",
            )
            return

        self._fallback_on_duration = datetime.timedelta(
            seconds=(keep_alive.total_seconds() * 20 * fallback_on_ratio)
        )
        self._fallback_off_duration = datetime.timedelta(
            seconds=(keep_alive.total_seconds() * 20 * (1 - fallback_on_ratio))
        )
        _LOGGER.info(
            (
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if self.fallback_force_switch_entity_id:
            self.async_on_remove(
                async_track_state_change_event(
                    self.hass,
                    [self.fallback_force_switch_entity_id],
    ...
    async def _async_control_heating(self, time=None, force=False) -> None:
        """
        Check if we need to turn heating on or off. If temperature is unavailable
        run climate in fallback mode
        """
        if self._is_fallback_mode_enabled:
            await self._async_fallback_mode()
        else:
    ...
    def _async_generate_attributes(self) -> tuple[str, dict[str, Any]]:
        ...
        attr["fallback_mode"] = (
            STATE_ON if self._is_fallback_mode_enabled else STATE_OFF
        )
        return state, attr
    ...
    async def _async_fallback_mode(self) -> None:
    ...
    @property
    def _is_fallback_mode_enabled(self) -> bool:
        """If climate should run in fallback mode."""
        if not self._sensor_available:
            return True
        if self._is_fallback_force_switch_set:
            return True
        return False

    @property
    def _is_fallback_force_switch_set(self) -> bool:
        """If force fallback mode is currently active."""
        if not self.fallback_force_switch_entity_id:
            return False

        if not self.hass.states.get(self.fallback_force_switch_entity_id):
            return False

        return self.hass.states.is_state(self.fallback_force_switch_entity_id, STATE_ON)
